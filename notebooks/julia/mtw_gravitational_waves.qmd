...
In the weak-field limit, the Einstein Field Equations linearize to:
$\square \bar{h}_{ab} = -16\pi G T_{ab}$

where $\bar{h}_{ab} = h_{ab} - \frac{1}{2}\eta_{ab} h$ is the trace-reversed
perturbation. In a vacuum ($T_{ab}=0$) and using the Lorenz gauge, this
