"""xAct expression builder for Elegua adapters.

Translates xAct-specific actions (DefManifold, DefTensor, ToCanonical, etc.)
into Wolfram expression strings. Injected into Elegua's OracleAdapter as::

    from sxact.elegua_bridge import build_xact_expr

    adapter = OracleAdapter(expr_builder=build_xact_expr)

Moved from elegua/tests/xact_builder.py (reference implementation) into sxact
as domain-owned code (sxAct-fkug).
"""
